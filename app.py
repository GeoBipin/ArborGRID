...
<style>
    ...
    [data-testid="metric-container"] label {
        color: #8b9ab0 !important; font-size: 0.78rem !important;
        font-weight: 600 !important; letter-spacing: 0.06em !important;
        text-transform: uppercase;
    }
    [data-testid="metric-container"] [data-testid="stMetricValue"] {
        color: #e8f0fe !important; font-size: 1.6rem !important;
    ...
    .stTabs [data-baseweb="tab-list"] {
        background-color: #1a2030; border-radius: 8px;
        padding: 4px; gap: 4px;
    }
    .stTabs [data-baseweb="tab"] {
        border-radius: 6px; color: #8b9ab0; font-weight: 500;
    }
...
# --- Custom Load for Trees (Google Drive Bypass) ---
if TREES_F.exists():
    gdf_trees = load_gdf(str(TREES_F))
else:
    try:
        # This uses the TREES_URL you defined in the CONFIG section
        gdf_trees = load_gdf(TREES_URL)
    except Exception as e:
        st.error(f"Failed to load tree data from Drive: {e}")
        gdf_trees = None
...
with st.sidebar:
    ...
    <div style='text-align:center; padding:8px 0 16px 0;'>
        ...
        <div style='font-size:1.3rem; font-weight:700; color:#e8f0fe;'>
            ArborGRID
        </div>
        <div style='font-size:0.75rem; color:#8b9ab0; margin-top:4px;'>
            Urban Vegetation & Utility Risk Sentinel
        </div>
    </div>
    """, unsafe_allow_html=True)
    st.divider()
    ...
    for num, label, color, active in [
        ...
        st.markdown(f"""
        <div style='background:{"#1a2a1a" if active else "#141a22"};
                    border:1px solid {color if active else "#2e3a4e"};
                    border-radius:8px; padding:8px 12px; margin-bottom:6px;
                    display:flex; align-items:center; gap:10px;'>
            <span style='font-size:0.7rem;font-weight:700;
                         color:{color};min-width:18px;'>P{num}</span>
            <span style='font-size:0.82rem;color:#c8d0da;'>{label}</span>
            <span style='margin-left:auto;font-size:0.8rem;'>{icon}</span>
        </div>
        """, unsafe_allow_html=True)
    ...
    if has_height_alerts:
        ...
        <div style='background:#2a1515;border:1px solid #e74c3c44;
                    ...
                    font-size:0.78rem;color:#d0a0a0;line-height:1.6;'>
...
<div style='padding:8px 0 20px 0;'>
    <h1 style='margin:0;font-size:1.8rem;font-weight:700;color:#e8f0fe;'>
        🌳⚡ ArborGRID {phase_badge}
    </h1>
    <p style='margin:6px 0 0 0;color:#6b7a8d;font-size:0.88rem;'>
        Vancouver, BC &nbsp;·&nbsp; 15 Metre Tree Corridors &nbsp;·&nbsp;
        Basemap: <b style='color:#a0b0c8;'>{selected_basemap}</b>
    </p>
</div>
""", unsafe_allow_html=True)
    ...
    with t1:
        if gdf_ready(gdf_lines) and "risk_level" in gdf_lines.columns:
            ...
            for col, lab, clr in zip(
                ...
                col.markdown(
                    f"<div style='background:#1a2030;"
                    f"border:1px solid {clr}44;border-radius:10px;"
                    f"padding:14px;text-align:center;'>"
                    f"<div style='color:{clr};font-size:1.5rem;"
                    f"font-weight:700;'>{n:,}</div>"
                    f"<div style='color:#8b9ab0;font-size:0.72rem;"
                    f"margin-top:4px;text-transform:uppercase;"
                    f"letter-spacing:0.06em;'>{lab}</div></div>",
                    unsafe_allow_html=True,
                )
...
if not _d_df_all.empty:
    ...
    _d_filter_label = (
        f'<span style="color:#f39c12;font-size:0.82rem;">'
    ...
    <div style='padding:8px 0 18px 0;'>
        <h2 style='margin:0;font-size:1.6rem;font-weight:700;color:#e8f0fe;'>
            🌳 Vancouver Urban Forest Scientific Dashboard
            &nbsp;&nbsp;{_d_filter_label}
        </h2>
        <p style='margin:6px 0 0 0;color:#8b9ab0;font-size:0.88rem;'>
